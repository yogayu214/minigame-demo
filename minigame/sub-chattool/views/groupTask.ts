...
export default function (PIXI: any, app: any, _query: any) {
  ...
  const titleText = p_text(PIXI, {
    content: '聊天工具 - 群活动', fontSize: r(20), fill: 0x000000, x: r(26), y: r(60), fontWeight: 'bold',
  });

  // ========== 任务列表 ==========
  const taskListBox = p_box(PIXI, { width: contentWidth, height: r(413), x: r(26), y: r(120), radius: r(8) });
  const taskList = p_scroll(PIXI, { width: contentWidth, height: r(413) });
  ...
  // ========== 创建任务按钮 ==========
  const createGroupTaskBtn = p_button(PIXI, { width: r(196), height: r(48), x: (r(375) - r(196)) / 2, y: r(560), radius: r(4), color: 0x07c160 });
  ...
  // ========== 刷新按钮 ==========
  const refreshBtn = p_button(PIXI, { width: r(196), height: r(48), x: (r(375) - r(196)) / 2, y: r(620), radius: r(4), color: 0x07c160 });
