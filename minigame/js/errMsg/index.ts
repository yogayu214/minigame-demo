module.exports = {
  errMsgGlobal: {
    'fail file already exists': (obj: any) => {
      obj.callback(`错误：存在同名文件或目录`);
    },
    'fail tempFilePath file not exist': (obj: any) => {
    ...
    'fail directory not empty': (obj: any) => {
      obj.callback(`错误：目录不为空`);
    },
    ERR_PARENT_DIR_NOT_EXISTS: (obj: any) => {
      obj.callback(`错误：文件所在目录不存在请去添加目录`);
    },
    MEDIA_ERR_NETWORK: (obj: any) => {
      obj.callback(`当下载时发生错误`);
    },
    MEDIA_ERR_DECODE: (obj: any) => {
      obj.callback(`解码错误`);
    },
    MEDIA_ERR_SRC_NOT_SUPPORTED: (obj: any) => {
